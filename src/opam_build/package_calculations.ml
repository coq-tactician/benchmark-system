let calculate ~root_dir () =
  ...
  print_endline "\n";
  let base_compiler = OpamPackage.create (OpamPackage.Name.of_string "ocaml-base-compiler") (OpamPackage.Version.of_string "4.08.1") in
  let coq = OpamPackage.create (OpamPackage.Name.of_string "coq") (OpamPackage.Version.of_string "8.11.2") in
  let pkgs = OpamPackage.Set.add base_compiler @@ OpamPackage.Set.add coq pkgs in
